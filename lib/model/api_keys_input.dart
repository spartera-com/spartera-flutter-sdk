...
class ApiKeysInput {
  ...
  @override
  String toString() => 'ApiKeysInput[userId=$userId, companyId=$companyId, roleId=$roleId, name=$name, expirationDateUtc=$expirationDateUtc]';
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "ApiKeysInput[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "ApiKeysInput[$key]" has a null value in JSON.');
        });
        return true;
      }());
